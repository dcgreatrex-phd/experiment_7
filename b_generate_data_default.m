function b_generate_data_default(subNo)
...
%     
    ...
    %create a new variable to place the pair score.
    ds2.a_pair_score = 0;

    % Now create a unique pair score that I can use later to filter out the
    % duplications in the final stimulus choices.
    n_combin_pairs=length(finallist); 
    for i=1:n_combin_pairs;        
    
        %find the indices of the rule in the matrix
        %then sum the array elements in the data
       a = finallist(i,1);
       b = finallist(i,2);   
       row_id=find(sum(ismember(finallist,[a b]),2)>1);
    
       %now loop through the row id pairing as an index of finallist and use
       %this to add a new variable number to both positions in finallist.
       for q = 1:2;
           if ds2.a_pair_score(row_id(q)) == 0;
               ds2.a_pair_score(row_id(q)) = i;
           end    
       end 
    end

    %%%%ds2 = sortrows(ds2,{'pair_score'}); 
...
%     % 5.2 - Join both databases on the relevant keys and rename in order to get the final combination list
    ...
    ds3.Properties.VarNames{4} = 'g_filename1';
    ds3.Properties.VarNames{5} = 'h_soundname1';
    ds3.Properties.VarNames{6} = 'i_genre1';
    ds3.Properties.VarNames{7} = 'j_value1';
    ds3.Properties.VarNames{8} = 'k_famil1';
    ...
    ds4 = ds4(:, sort(ds4.Properties.VarNames) ); 
    ...
    % 6.3.2 - Remove any rows where the individual familiar is less or equal to than 3;
    for i = 1:size(ds4,1);
    ...
    ds4.Properties.VarNames{12} = 'l_difference_a';
    ...
    % 7.5 - Sort data table assending on total value scores.
    ds4 = sortrows(ds4,{'randomno'});    
    
    
    
    
    
    % Now delete any pair score duplicates in the order that are presented
    % within the randmon distribution
    ...
    % Now create a unique pair score that I can use later to filter out the
    % duplications in the final stimulus choices.
    n_combin_pairs2=length(ds4); 
    for i=1:n_combin_pairs2;        
    
        %find the indices of the rule in the matrix
        %then sum the array elements in the data
        c = find(ds4.a_pair_score==ds4.a_pair_score(i)); 
    ...
    % delete all of the the flag = 2 rows. 
    for i=1:size(ds4,1);   
        if ds4.flag(i) == 2;          
            delete4(i)=true;          
        end
    ...
    %7.2 - Remove flag and pair_score variable ;
    ds4(:,15) = [];
    ds4(:,1) = [];
    ...
    %%% At this stage, sort data ub unique pairings after random shuffle
    %%% and delete the second on of the pair before adding unique condition
    %%% ids to the data.
